...
** Optimizing pools, ppn, and pools/ppn ratio
#+BEGIN_SRC python
...
import matplotlib.pyplot as plt
import numpy as np
import ase_addons.bulk
from ase.utils.eos import EquationOfState
from ase.lattice.spacegroup import crystal
from ase_addons.bulk import rhombo_rocksalt

# Set QE PPs
Espresso.PPs = QE_PSEUDO
ESPRESSORC['PPpath'] = '/home-research/zhongnanxu/pseudopotentials/qe_pseudo'
...
bulk = rhombo_rocksalt(['Ni', 'O'], a=2.09, b=4.188, mags=(0.5, 0), primitive=False)
ppns, ppns_time = [], []

for ppn, pools in params:
    with Espresso('process-pools-test/NiO-2x2x2/{0}-process/{1}-pools'.format(ppn, pools),
                  atoms=bulk,
                  # CONTROL
                  disk_io='none',
                  # SYSTEM
                  ...
                  # CELL
                  kpts=(4, 4, 4),
                  walltime='160:00:00', ppn=ppn, pools=pools) as calc:
        try:
            calc.calculate()
            time = calc.get_cputime()
            print ppn, time
            plt.plot(ppn, time, marker='$%d$' % (ppn / pools), ms=12, c='k')
        except (EspressoSubmitted, EspressoRunning):
            print calc.espressodir, 'running'
        except (EspressoNotConverged):
            print calc.espressodir, 'not converged'
...
#+RESULTS:
#+begin_example
process-pools-test/NiO-2x2x2/1-process/1-pools running
process-pools-test/NiO-2x2x2/2-process/1-pools running
4 3115.0
4 3064.3
8 2066.8
8 1654.3
...
From these results, it's clear one wants to keep the ppn / pools between 4 and 8. For 16 nodes, seeting pools to 2 seems alright.
